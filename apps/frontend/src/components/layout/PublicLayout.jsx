...
const PublicLayout = ({ children, user, logout }) => {
  ...
  const isAuthPage = isLoginPage || isSignupPage || isResetPage;
    ...
    <div className="min-h-screen bg-transparent">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 lg:px-6">
          <Link className="flex items-center gap-3" to="/">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-700">AI Resume Studio</p>
          ...
          <div className="flex items-center gap-3">
            {user && isAuthPage ? null : user ? (
              ...
              <Link className="rb-btn-primary px-4 py-2" to="/signup">
                Create Account
              </Link>
            ) : isSignupPage || isResetPage ? (
              <Link className="hidden text-sm font-semibold text-teal-700 underline decoration-2 underline-offset-4 transition hover:text-teal-900 sm:inline-flex" to="/login">
                Sign In
              </Link>
            ) : (
              <>
      ...
      <footer className="border-t border-slate-200/80 bg-white/70">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-8 text-sm text-slate-500 lg:flex-row lg:items-center lg:justify-between lg:px-6">
          <p>Resume Builder for web and desktop workflows.</p>
